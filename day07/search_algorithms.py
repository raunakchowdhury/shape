...
def best_first(state, heuristic = misplaced_heuristic): #also greedy search
    ...
    while not goal_test(popped_state[1]) and not len(frontier) == 0:
        ...
        heuristics = list(children_heuristic.keys())
        heuristics.sort()
        #heuristics.reverse()
        #print(heuristics)

        #print('Before:', len(child_states))
        heuristic_child_states = []
        for index in heuristics:
            for state in children_heuristic[index]:
                heuristic_child_states.append(state)
        #print('After:', len(child_states))

        for child_state in heuristic_child_states:
            #print(not child_state[1] in seen)
            if not child_state[1] in seen:
                seen.add(child_state[1])
                parents[child_state] = popped_state
...
if __name__ == "__main__":
    ...
    start = time.time()
    solution, states_expanded, max_frontier = best_first(test_state, manhattan_heuristic)
    end = time.time()
    print_result(solution, states_expanded, max_frontier)
    print("Total time: {0:.3f}s".format(end-start))
