...
def compute_utility(board, color, current_depth=0):
    ...
    heuristics_second_half = [(edge_heuristic, 2.0), (corner_heuristic,1.0),(sweet_sixteen_heuristic, 1.0), (invincible_edge_heuristic,1.0)]#,(sweet_sixteen_heuristic, 1.0)
    heuristic_value = 0
...
def corner_heuristic(board,color):
        ...
        if board[row][column] == color:
            ...
            for (adjacent_row,adjacent_column) in adjacent_corners[(row,column)]:
                if board[adjacent_row][adjacent_column] == color:
                    heuristic_value -= 100
                elif board[adjacent_row][adjacent_column] == opposing_color:
                    heuristic_value += 1000
    ...
    return heuristic_value
'''
def deny_corner(board,color,best_move_list,move=None):
    corners = [(0,0),(0,len(board)-1),(len(board)-1,0),(len(board)-1,len(board)-1)]
    adjacent_corners = {
        (0,0) : [(0,1),(1,0)],
        (0,len(board)-1): [(0,len(board)-2), (1,len(board)-1)],
        (len(board)-1,0): [(len(board)-2,0), (len(board)-2,1)],
        (len(board)-1,len(board)-1): [(len(board)-2,len(board)-2), (len(board)-1,len(board)-2)]
    }
    corner_count = 0
    opposing_corner_count = 0
    chosen_move = 0
    ...
    for (row,column) in corners:
        if board[row][column] == color:
            ...
            opposing_corner_count += 1

    for move_index in range(len(best_move_list)):
        sys.stderr.write('Testing move {} {}'.format(best_move_list[0],best_move_list[1]) )
        new_board = play_move(board ,color,best_move_list[0],best_move_list[1])
        for (row,column) in corners:
            if new_board[row][column] == color and board[row][column] != color:
                return move_index
    return 0
'''
...
def select_move_minimax(board, color):
    ...
    if len(moves) != 0:
        for (column,row) in moves:
            ...
            if not utility in move_utility.keys():
                move_utility[utility] = [(column,row,board)]
            else:
                move_utility[utility].append((column,row))
            sys.stderr.write('Move {},{} with utility {} input!\n'.format(column,row,utility))
    new_max = max(move_utility.keys())
    chosen_move = deny_corner(board, color, move_utility[new_max])    #random.randint(0,len(move_utility[new_max])-1)
    sys.stderr.write('Move index: {} of length {}\n'.format(chosen_move, len(move_utility[new_max])))
    selected_move = move_utility[new_max][chosen_move]
    sys.stderr.write('Selected move weight: {} of {}\n# Moves Available: {}\n Move Used:{}\n'.format(new_max,move_utility,len(moves),selected_move))
...
def run_ai():
    ...
    """
    print("Totally Not Hardcoded") # First line is the name of this AI
